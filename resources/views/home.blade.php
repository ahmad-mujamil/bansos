...
@section('content')
    <style>
        ...
        .dash-card a, .dash-card a:hover, .dash-card a:focus { color: inherit; text-decoration: none; }
        .dash-card { transition: transform .12s ease, box-shadow .12s ease; }
        .dash-card:hover { transform: translateY(-2px); box-shadow: 0 12px 26px rgba(15, 23, 42, .20); }
        .dash-head-link { display: block; cursor: pointer; }
        .dash-row--link { cursor: pointer; border-radius: 7px; padding: .2rem .35rem; margin: 0 -.35rem; transition: background .12s ease; }
        .dash-row--link:hover { background: rgba(255, 255, 255, .20); }
    </style>
        ...
        <div class="row g-3 mb-4">
            <div class="col-12 col-md-4">
                <div class="card dash-card dash-indigo h-100">
                    <div class="card-body">
                        <a class="dash-head-link" href="{{ route('dashboard.detail', ['penerima' => 'perorangan']) }}">
                            <div class="dash-title">Perorangan</div>
                            <div class="dash-value">{{ number_format($totalPerorangan) }}</div>
                        </a>
                        <div class="dash-rows">
                            <a class="dash-row dash-row--link" href="{{ route('dashboard.detail', ['penerima' => 'perorangan', 'jenis' => 'bansos']) }}">
            ...
            <div class="col-12 col-md-4">
                <div class="card dash-card dash-green h-100">
                    <div class="card-body">
                        <a class="dash-head-link" href="{{ route('dashboard.detail', ['penerima' => 'organisasi']) }}">
                            <div class="dash-title">Organisasi</div>
                            <div class="dash-value">{{ number_format($totalOrganisasi) }}</div>
                        </a>
                        <div class="dash-rows">
                            <a class="dash-row dash-row--link" href="{{ route('dashboard.detail', ['penerima' => 'organisasi', 'jenis' => 'hibah']) }}">
                                <span class="dash-row__label">Hibah</span>
                                <span class="dash-row__value">{{ number_format($orgHibah) }}</span>
                            </a>
                            <a class="dash-row dash-row--link" href="{{ route('dashboard.detail', ['penerima' => 'organisasi', 'jenis' => 'bantuan_kelompok']) }}">
                                <span class="dash-row__label">Bantuan ke Masyarakat</span>
                                <span class="dash-row__value">{{ number_format($orgKelompok) }}</span>
                            </a>
                            @if ($orgBansos > 0)
                                <a class="dash-row dash-row--link" href="{{ route('dashboard.detail', ['penerima' => 'organisasi', 'jenis' => 'bansos']) }}">
                                    <span class="dash-row__label">Bantuan Sosial</span>
            ...
            <div class="col-12 col-md-4">
                <div class="card dash-card dash-purple h-100">
                    <div class="card-body">
                        <a class="dash-head-link" href="{{ route('dashboard.detail') }}">
                            <div class="dash-title">Pengajuan</div>
                            <div class="dash-value">{{ number_format($totalPengajuan) }}</div>
                        </a>
                        <div class="dash-rows">
                            <a class="dash-row dash-row--link" href="{{ route('dashboard.detail', ['jenis' => 'bansos']) }}">
                                <span class="dash-row__label">Bantuan Sosial</span>
                                <span class="dash-row__value">{{ number_format($pengBansos) }}</span>
                            </a>
                            <a class="dash-row dash-row--link" href="{{ route('dashboard.detail', ['jenis' => 'hibah']) }}">
                                <span class="dash-row__label">Hibah</span>
                                <span class="dash-row__value">{{ number_format($pengHibah) }}</span>
                            </a>
                            <a class="dash-row dash-row--link" href="{{ route('dashboard.detail', ['jenis' => 'bantuan_kelompok']) }}">
                                <span class="dash-row__label">Bantuan ke Masyarakat</span>
